...
int addset(bool* sr, bool* s1, bool* s2, int n) /*或（+）运算*/
{
	int i;
	for (i = 0; i < n; i++)
	{
...
int subset(bool* sr, bool* s1, bool* s2, int n) /*减运算*/
{
	int i;
	for (i = 0; i < n; i++)
	{
...
int mulset(bool* sr, bool* s1, bool* s2, int n)/*逻辑乘*/
{
	int i;
	for (i = 0; i < n; i++)
	{
...
int getch()
{
	if (cc == ll)
	{
		if (feof(fin))  // feof()函数用来检测当前文件流上的文件结束标识，判断是否读到了文件结尾
		{
...
* 通过 enum symbol sym 传递单词的类别
* 通过 char id[al+1] 传递标识符单词的值
* 通过 int num 传递常亮单词的值
*/
int getsym()
{
...
void enter(enum object k, int* ptx, int lev, int* pdx)
{
	...
	printf("当前的table：\n");
	for (int i = 1; i < (*ptx) + 1; i++)
	{
		printf("ptx:%d\tname:%s\tkind:%d\tval:%d\tlevel:%d\tadr:%d\tsize:%d\n", i,
			table[i].name, table[i].kind, table[i].val, table[i].level, table[i].adr, table[i].size);
	}
	printf("\n\n");
